      ...
      <el-form :model="state.goodForm"
        ...
        <el-form-item>
          <el-button type="primary"
                     @click="submitAdd()">{{ state.id ? '立即修改' : '立即创建' }}</el-button>
        </el-form-item>
      </el-form>
    </el-card>
  </div>
...
let instance
// 在setup()函数中使用onMounted()生命周期钩子，可以在组件挂载后执行一些初始化操作，比如获取数据、绑定事件等。
// onMounted()生命周期钩子可以用于替代Vue2中的mounted生命周期钩子，但是它的执行时机更早，即在组件实例创建完成后立即执行。
// onMounted()生命周期钩子接收一个函数作为参数，该函数会在组件挂载后立即执行。
// onMounted()生命周期钩子的返回值可以是一个函数，该函数会在组件卸载前执行，用于清理一些操作。
onMounted(() => {
